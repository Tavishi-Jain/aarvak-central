    ...
    <BoardLayout topbar={<div className="flex w-full items-center justify-between"><span className="font-display text-sm font-bold tracking-sign text-chalk">AARVAK TSJ 2026 DASHBOARD</span><span className="label text-lamp">CHAT</span></div>}>
      ...
      <BoardPanel padded={false}>
        ...
        <form onSubmit={send} className="flex gap-2 border-t border-seam p-panel">
          ...
          <input id="chat-message" value={draft} onChange={event => setDraft(event.target.value)} disabled={!DEMO_MODE} placeholder={DEMO_MODE ? 'Write a demo message…' : 'Chat is unavailable until deployed'} className="min-w-0 flex-1 rounded-slot border border-seam bg-recess px-3 py-2 text-sm text-chalk placeholder:text-dim" />
          <button type="submit" disabled={!DEMO_MODE || !draft.trim()} className="rounded-slot bg-lamp px-3 text-void transition-transform hover:scale-105 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Send message"><Send size={16} aria-hidden="true" /></button>
        </form>
      </BoardPanel>
    </BoardLayout>
  )
